...
Hello! I'm a fifth-year PhD candidate in CMU's ECE program working with [Prof. Akshitha Sriraman](https://users.ece.cmu.edu/~asrirama/). My research passion lies in finding novel ways to make cloud/web systems more sustainable by targeting more efficient use of data center resources (particularly hardware) in order to reduce carbon emissions, e-waste, and other environmental impacts from hyperscale systems. This spans the compute stack — from designing low-carbon servers (my GreenSKU work, an IEEE Micro Top Pick now being adopted across Microsoft Azure) to extending the useful lifetime of existing servers through intelligent scheduling.
